...
def _ensure_task_progress(conn: Any) -> None:
    conn.execute(
        """
...
def build_question_skill_map(conn: Any, track_id: str = "") -> dict[str, Any]:
    ...
    for row in rows:
        ...
        conn.execute(
            ...
            """,
            (row["id"], best.get("certification") or row.get("track_id") or track_id or "", best.get("domain_id") or "", best.get("id") or "", confidence, json.dumps({"matches": matches[:3], "source": "heuristic_v7"})),
        )
        inserted += 1
    return {"questions_scanned": len(rows), "mapped": inserted, "low_confidence": low_confidence}


def _mapping_resolution(conn: Any, track_id: str, questions: list[dict[str, Any]]) -> tuple[dict[str, dict[str, Any]], dict[str, int]]:
    ...
    for row in questions:
        ...
        if edge:
            ...
            resolved[row["id"]] = {"skill_id": edge["skill_id"], "domain_id": edge.get("domain_id") or skills[edge["skill_id"]].get("domain_id"), "provenance": provenance, "confidence": float(edge.get("confidence") or 0)}
            stats[provenance] += 1
            continue
        matches = matched_skills(_question_text(row), row.get("track_id") or track_id, limit=1)
        if matches and matches[0].get("id") in skills:
            best = matches[0]
            resolved[row["id"]] = {"skill_id": best.get("id"), "domain_id": best.get("domain_id"), "provenance": "heuristic_fallback", "confidence": float(best.get("confidence") or 0)}
...
def _lab_skill_activity(conn: Any, track_id: str) -> dict[str, dict[str, int]]:
    ...
    for event in conn.execute("SELECT event_type, metadata_json, COUNT(*) AS count FROM learning_events WHERE (? = '' OR track_id = ?) GROUP BY event_type, metadata_json", (track_id or "", track_id or "")):
...
def skill_mastery(conn: Any, track_id: str = "snowpro-core") -> dict[str, Any]:
    ...
    completed = {
        row["skill_id"]
        for row in conn.execute("SELECT skill_id FROM certification_task_progress WHERE track_id = ? AND completed = 1", (track_id,))
    }
    ...
    for skill in skills:
        ...
        level = 1  # Configured written task content always exists in the certification product.
        evidence = ["task_content_available"]
        if task_completed:
            level = max(level, 2)
        ...
        output.append({
            "skill_id": skill_id, "skill": skill.get("title"), "domain_id": skill.get("domain_id"), "domain": skill.get("domain"), "objective": skill.get("objective"),
            "mastery_level": level, "mastery_status": status, "evidence": evidence, "task_completed": task_completed,
            "question_count": len(q_rows), "mapped_question_count": len(q_rows), "attempts": attempts, "correct_attempts": correct, "misses": misses, "accuracy_pct": accuracy,
            "timed_attempts": timed_attempts, "timed_accuracy_pct": timed_accuracy, "available_labs": lab.get("available_labs", 0), "passed_labs": lab.get("passed_labs", 0),
            "mapping_provenance": dict(provenance_counts), "exam_traps": skill.get("exam_traps") or [], "aliases": skill.get("aliases") or [],
        })
    domain_rows: dict[str, dict[str, Any]] = {}
    for row in output:
        domain_id = row.get("domain_id") or row.get("domain") or "other"
        item = domain_rows.setdefault(domain_id, {"domain_id": domain_id, "domain": row.get("domain"), "skills": 0, "avg_mastery": 0, "attempts": 0, "correct_attempts": 0, "accuracy_pct": 0, "blockers": 0})
        item["skills"] += 1; item["avg_mastery"] += row["mastery_level"]; item["attempts"] += row["attempts"]; item["correct_attempts"] += row["correct_attempts"]
        if row["mastery_level"] < 4: item["blockers"] += 1
    ...
    mapped = len(resolved)
    reliable = int(mapping_stats.get("human_reviewed", 0)) + int(mapping_stats.get("persisted_high_confidence", 0))
    ...
    return {"track_id": track_id, "skills": output, "domains": list(domain_rows.values()), "mapping_stats": mapping_stats, "mapping_trust_pct": mapping_trust, "mapped_questions": mapped, "total_questions": len(questions)}


def readiness_model(conn: Any, track_id: str = "snowpro-core", mastery: dict[str, Any] | None = None) -> dict[str, Any]:
    cert = next((cert for cert in configured_skill_map().get("certifications") or [] if cert.get("id") == track_id), None)
    mastery = mastery or skill_mastery(conn, track_id)
    skills = mastery.get("skills") or []
    if not skills:
        return {"track_id": track_id, "status": "insufficient_data", "readiness_score": 0, "readiness_confidence": 0, "readiness_confidence_status": "insufficient", "blockers": ["No skill map found"], "next_actions": []}
    ...
    if best_mock >= 80: evidence_score += 10
    if misses > 40: evidence_score -= 8
    if repeated_miss: evidence_score -= min(12, len(repeated_miss) * 2)
    ...
    confidence = int(_clamp(mapping_trust * 0.35 + attempt_confidence * 0.25 + question_skill_coverage * 0.20 + mock_confidence * 0.15 + task_confidence * 0.05))
    confidence_status = "strong" if confidence >= 80 else "moderate" if confidence >= 55 else "weak" if confidence >= 30 else "insufficient"
    confidence_reasons = [f"Question-to-skill mapping trust: {mapping_trust}%", f"{attempts} mapped question attempts across {question_skill_coverage}% of skills", f"{mock_attempts} persisted timed mock exams", f"{task_completed}/{len(skills)} written task lessons completed"]

    blockers = []
    if attempts < 100: blockers.append(f"Question evidence is thin: {attempts} mapped attempts; target at least 100 before trusting readiness.")
    if mock_attempts < 2: blockers.append("Complete at least two timed mock exams.")
    if best_mock and best_mock < 80: blockers.append(f"Best timed mock score is {best_mock}%; target 80%+.")
    if low_skills: blockers.append(f"{len(low_skills)} skills are below accurate mastery level.")
    if repeated_miss: blockers.append(f"{len(repeated_miss)} skills show repeated misses under 70% accuracy.")
    if lab_available and lab_passed < min(3, lab_available): blockers.append(f"Only {lab_passed}/{lab_available} available labs are proven.")
    if mapping_trust < 65: blockers.append(f"Evidence mapping trust is only {mapping_trust}%; review or strengthen question mappings.")
    if score >= 82 and not blockers: status = "exam_ready"
    elif score >= 70: status = "near_ready"
    elif score >= 45: status = "needs_repair"
    elif attempts or task_completed: status = "learning"
    else: status = "not_started"
    weak = sorted(low_skills, key=lambda row: (row["mastery_level"], row["accuracy_pct"], -row["misses"]))[:5]
    next_actions = []
    for row in weak[:3]:
        action = "Start drill" if row["attempts"] < 5 else "Complete lab" if row["available_labs"] and not row["passed_labs"] else "Repair misses" if row["misses"] else "Practice"
    ...
    return {
        "track_id": track_id, "certification_title": (cert or {}).get("title", track_id), "status": status, "readiness_score": score,
        "readiness_confidence": confidence, "readiness_confidence_status": confidence_status, "confidence_reasons": confidence_reasons,
        "pass_probability_range": [max(0, score - 8), min(99, score + 6)], "attempts": attempts, "accuracy_pct": accuracy, "misses": misses,
        "avg_mastery_level": round(avg_level, 1), "mock_exam_attempts": mock_attempts, "best_mock_score": best_mock, "lab_passed": lab_passed, "lab_available": lab_available,
        "task_completed": task_completed, "mapping_trust_pct": mapping_trust, "blockers": blockers, "next_actions": next_actions, "domains": mastery["domains"], "weak_skills": weak,
    }


def _resolved_skill_for_row(resolved: dict[str, dict[str, Any]], row: dict[str, Any], skills: dict[str, dict[str, Any]]) -> dict[str, Any]:
    assignment = resolved.get(row["id"])
...
def mistake_queue(conn: Any, track_id: str = "snowpro-core", limit: int = 20) -> dict[str, Any]:
    ...
    items = []
    for row in rows:
        if int(row.get("missed_attempts") or 0) <= 0: continue
        skill = _resolved_skill_for_row(resolved, row, skills)
        text = _question_text(row).lower()
        mistake_type = "concept_gap"
        if any(term in text for term in ["except", " not ", "least", "most likely"]): mistake_type = "exam_trap"
        elif any(term in text for term in ["auto_suspend", "auto resume", "warehouse", "cache"]): mistake_type = "feature_confusion"
        elif any(term in text for term in ["syntax", "create", "grant", "copy into", "flatten"]): mistake_type = "syntax_or_command_gap"
        elif int(row.get("timed_attempts") or 0) and int(row.get("missed_attempts") or 0) >= int(row.get("correct_attempts") or 0): mistake_type = "time_pressure_error"
        items.append({"question_id": row["id"], "question": row.get("question"), "test_title": row.get("test_title"), "misses": int(row.get("missed_attempts") or 0), "attempts": int(row.get("attempts") or 0), "last_attempted": row.get("last_attempted"), "mistake_type": mistake_type, "skill_id": skill.get("id"), "skill": skill.get("title"), "domain": skill.get("domain"), "repair_action": repair_action_for_mistake(mistake_type, skill)})
...
def repair_action_for_mistake(mistake_type: str, skill: dict[str, Any]) -> str:
    ...
    if mistake_type == "exam_trap": return f"Review the exam traps for {skill_name}, then retry a targeted drill."
    if mistake_type == "feature_confusion": return f"Compare the decision rules for {skill_name}, then drill the task."
    if mistake_type == "syntax_or_command_gap": return f"Complete the build exercise for {skill_name}, then retest."
    if mistake_type == "time_pressure_error": return f"Run a timed mock after repairing {skill_name}."
...
def diagnostic_plan(conn: Any, track_id: str = "snowpro-core", count: int = 30) -> dict[str, Any]:
    ...
    for row in questions:
        ...
        item = dict(row); item["skill_id"] = skill.get("id"); item["skill"] = skill.get("title"); item["domain"] = domain
        by_domain[domain].append(item)
    domains = list(by_domain)
    if not domains: return {"track_id": track_id, "question_ids": [], "domains": [], "message": "No mapped questions available for diagnostic."}
    selected = []
    ...
    return {"track_id": track_id, "mode": "diagnostic", "target_count": count, "question_count": len(selected), "question_ids": [row["id"] for row in selected], "domains": [{"domain": domain, "available": len(rows)} for domain, rows in by_domain.items()], "questions": [{"id": row["id"], "question": row.get("question"), "domain": row.get("domain"), "skill": row.get("skill"), "test_title": row.get("test_title")} for row in selected]}


def command_brief(conn: Any, track_id: str = "snowpro-core", readiness: dict[str, Any] | None = None, mistakes: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    if readiness["status"] in {"not_started", "learning"} and readiness["attempts"] < 30:
        mission.append({"type": "diagnostic", "title": "Run baseline diagnostic", "detail": "Answer a balanced diagnostic before trusting the plan.", "href": f"#/diagnostic?track_id={track_id}"})
    for action in readiness.get("next_actions", [])[:3]:
        href = f"#/exercises?track_id={track_id}&skill_id={action['skill_id']}" if action["action"] == "Complete lab" else f"#/drill?track_id={track_id}&skill_id={action['skill_id']}"
        mission.append({"type": "repair", "title": f"{action['action']}: {action['skill']}", "detail": action["reason"], "href": href})
    if mistakes["items"]:
        mission.append({"type": "mistake_review", "title": "Repair repeated misses", "detail": f"{mistakes['total_unresolved']} missed questions need repair.", "href": f"#/drill?track_id={track_id}"})
    if not mission:
        mission.append({"type": "readiness_exam", "title": "Take a timed mock exam", "detail": "No urgent repair blocker remains. Validate under exam conditions.", "href": f"#/mock?track_id={track_id}"})
...
def portfolio(conn: Any) -> dict[str, Any]:
    ...
    for cert in configured_skill_map().get("certifications") or []:
        ...
        rows.append({"track_id": cert.get("id"), "title": cert.get("title"), "exam_code": cert.get("exam_code"), "official": cert.get("official", False), "status": readiness.get("status"), "readiness_score": readiness.get("readiness_score"), "readiness_confidence": readiness.get("readiness_confidence"), "attempts": readiness.get("attempts"), "accuracy_pct": readiness.get("accuracy_pct"), "blockers": readiness.get("blockers", [])[:3]})
