...
DDL = (
    ...
    CREATE TABLE IF NOT EXISTS candidate_talent_profiles (
      candidate_id INTEGER PRIMARY KEY REFERENCES candidate_accounts(id) ON DELETE CASCADE,
    ...
    """,
    "CREATE INDEX IF NOT EXISTS idx_candidate_credentials_candidate ON candidate_credentials(candidate_id, verification_status, updated_at)",
    "CREATE INDEX IF NOT EXISTS idx_candidate_credentials_provider_badge ON candidate_credentials(provider, provider_badge_id)",
    "CREATE INDEX IF NOT EXISTS idx_credential_verification_events_credential ON credential_verification_events(credential_uid, checked_at)",
    "CREATE INDEX IF NOT EXISTS idx_candidate_talent_discoverable ON candidate_talent_profiles(recruiter_discoverable, public_profile, updated_at)",
)
...
def ensure_talent_schema() -> None:
    """Create the additive candidate credential/talent tables on SQLite and PostgreSQL.

    PostgreSQL also owns a versioned migration. This runtime bootstrap keeps
