...
const guestItems = [["#/certifications", "Certifications"], ["#/pricing", "Pricing"], ["#/about", "About"]];
const homeItems = certificationItems;
...
const aliases = new Map([
  ...
  ["#/diagnostic", "#/practice"], ["#/drill", "#/practice"], ["#/quiz", "#/practice"], ["#/mistakes", "#/practice"], ["#/due", "#/practice"], ["#/question-studio", "#/practice"], ["#/daily-session", "#/practice"], ["#/practice-hub", "#/practice"],
...
export async function renderNav() {
  ...
  nav.innerHTML = `<div class="v26-nav-inner">
    <a class="v26-brand" href="#/home" aria-label="Snowflake Study Command Center home"><span>Snowflake Brain</span><small>Study Command Center</small></a>
    ...
    <div class="v26-nav-actions">
      ...
      ${account ? `<div class="v26-account-menu"><button class="v26-account-link compact" type="button" data-account-trigger aria-label="${escapeHtml(account.display_name)} account menu" aria-haspopup="menu" aria-expanded="false"><span>${escapeHtml(initials)}</span></button><div class="v26-account-popover" data-account-popover role="menu" hidden><div><strong>${escapeHtml(account.display_name)}</strong><span>${escapeHtml(account.email)}</span><em>${escapeHtml(account.plan)}</em><small class="${account.email_verified === false ? "verification-required" : "verification-ok"}">${escapeHtml(verification)}${methods ? ` · ${escapeHtml(methods)}` : ""}</small></div>${account.admin_access ? `<a href="#/admin" role="menuitem">Founder Operations</a>` : ""}<a href="#/progress?track_id=${encodeURIComponent(selected)}" role="menuitem">Progress Dashboard</a><a href="#/practice-hub?track_id=${encodeURIComponent(selected)}" role="menuitem">Practice Hub</a><a href="#/daily-session?track_id=${encodeURIComponent(selected)}" role="menuitem">Daily Session</a><a href="#/question-studio?track_id=${encodeURIComponent(selected)}" role="menuitem">Question Studio</a><a href="#/adaptive?track_id=${encodeURIComponent(selected)}" role="menuitem">Adaptive Readiness</a><a href="#/due?track_id=${encodeURIComponent(selected)}" role="menuitem">Due Today</a><a href="#/mistakes?track_id=${encodeURIComponent(selected)}" role="menuitem">Mistake Notebook</a><a href="#/confidence?track_id=${encodeURIComponent(selected)}" role="menuitem">Confidence Calibration</a><a href="#/study-plan?track_id=${encodeURIComponent(selected)}" role="menuitem">Study Plan</a><a href="#/credentials?track_id=${encodeURIComponent(selected)}" role="menuitem">Verified Credentials</a><a href="#/account" role="menuitem">Account & Security</a><a href="#/settings" role="menuitem">Settings</a><a href="#/membership" role="menuitem">Membership</a><button type="button" data-auth-logout role="menuitem">Sign Out</button></div></div>` : `<button class="v26-login-link" type="button" data-auth-intent="login">Sign In</button><button class="v26-signup-link" type="button" data-auth-intent="signup">Create Account</button>`}
