...
export async function renderNav() {
  ...
  nav.innerHTML = `<div class="v26-nav-inner">
    ...
    <div class="v26-nav-actions">
      ...
      ${account ? `<div class="v26-account-menu"><button class="v26-account-link compact" type="button" data-account-trigger aria-label="${escapeHtml(account.display_name)} account menu" aria-haspopup="menu" aria-expanded="false"><span>${escapeHtml(initials)}</span></button><div class="v26-account-popover" data-account-popover role="menu" hidden><div><strong>${escapeHtml(account.display_name)}</strong><span>${escapeHtml(account.email)}</span><em>${escapeHtml(account.plan)}</em><small class="${account.email_verified === false ? "verification-required" : "verification-ok"}">${escapeHtml(verification)}${methods ? ` · ${escapeHtml(methods)}` : ""}</small></div><a href="#/account" role="menuitem">Account & Security</a>${account.email_verified === false ? `<a href="#/account" role="menuitem">Verify Email</a>` : ""}<a href="#/adaptive?track_id=${encodeURIComponent(selected)}" role="menuitem">Adaptive Readiness</a><a href="#/progress?track_id=${encodeURIComponent(selected)}" role="menuitem">My Progress</a><a href="#/membership" role="menuitem">Membership</a><button type="button" data-auth-logout role="menuitem">Sign Out</button></div></div>` : `<button class="v26-login-link" type="button" data-auth-intent="login">Sign In</button><button class="v26-signup-link" type="button" data-auth-intent="signup">Create Account</button>`}
