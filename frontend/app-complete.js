...
async function boot(){await refreshCandidate().catch(()=>{});renderCandidateAccess();await renderNav();renderFeedback();window.addEventListener("hashchange",handleRoute);window.addEventListener("track-change",()=>{renderNav();handleRoute()});window.addEventListener("candidate-change",()=>{renderNav();handleRoute()});window.addEventListener("theme-toggle",e=>{applyTheme(e.detail?.theme);renderNav()});if(!window.location.hash)window.location.hash="#/home";await handleRoute()}
boot();
