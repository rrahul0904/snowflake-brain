...
async function handleRoute(){await route();footer();document.querySelector("#view-root")?.focus({preventScroll:true})}
window.__setSnowflakeTheme=applyTheme;applyTheme(localStorage.getItem("snowflake-certification.theme")||"dark");
async function boot(){renderCandidateAccess();await renderNav();renderFeedback();window.addEventListener("hashchange",handleRoute);window.addEventListener("track-change",()=>{renderNav();handleRoute()});window.addEventListener("candidate-change",()=>{renderCandidateAccess();renderNav();handleRoute()});window.addEventListener("theme-toggle",e=>{applyTheme(e.detail?.theme);renderNav()});if(!window.location.hash)window.location.hash="#/home";await handleRoute();void refreshCandidate({notify:true}).catch(()=>{})}
boot();
