...
function applyTheme(theme){const next=theme==="dark"?"dark":"light";document.documentElement.dataset.theme=next;localStorage.setItem("snowflake-certification.theme",next)}
function footer(){const path=(window.location.hash||"#/home").split("?")[0];let root=document.querySelector("#v26-footer");if(!root){root=document.createElement("footer");root.id="v26-footer";document.body.appendChild(root)}if(path==="#/mock/session"){root.hidden=true;return}root.hidden=false;const t=encodeURIComponent(activeTrack());root.innerHTML=`<div class="v26-footer-inner"><div class="v26-footer-brand"><strong>Snowflake Certified</strong><p>Independent SnowPro certification preparation.</p><small>Independent certification-prep software.</small></div><nav aria-label="Footer navigation"><div><a href="#/curriculum?track_id=${t}">Curriculum</a><a href="#/practice?track_id=${t}">Practice</a><a href="#/reference?track_id=${t}">Reference</a><a href="#/journal?track_id=${t}">Journal</a><a href="#/about">About</a><a href="#/changelog">Changelog</a><a href="#/privacy">Privacy</a></div></nav></div>`}
async function handleRoute(){await route();await renderNav();footer();document.querySelector("#view-root")?.focus({preventScroll:true})}
window.__setSnowflakeTheme=applyTheme;applyTheme(localStorage.getItem("snowflake-certification.theme")||"light");
