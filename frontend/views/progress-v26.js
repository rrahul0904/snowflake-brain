...
export default async function mount(container, params = {}) {
  ...
  if (params.section === "mistakes") {
    requestAnimationFrame(() => container.querySelector("#mistakes")?.scrollIntoView({ block: "start" }));
  }
}
...
function learningCommandCenter(trackId, due, mistakes, calibration, plan) {
  ...
  return `<section class="v26-learning-command"><div><span>Due Today</span><strong>${Number(due.due_count || 0)}</strong><a href="#/practice?track_id=${encodeURIComponent(trackId)}&mode=srs">Review now →</a></div><div><span>Active Mistakes</span><strong>${openMistakes}</strong><a href="#/progress?track_id=${encodeURIComponent(trackId)}&section=mistakes">Open notebook →</a></div><div><span>Confidence</span><strong>${confidence}</strong><small>${escapeHtml(String(calibration.status || "insufficient_data").replaceAll("_", " "))}</small></div><div><span>Exam Date</span><strong>${escapeHtml(exam)}</strong><button type="button" data-edit-plan>Set plan</button></div></section>`;
}
