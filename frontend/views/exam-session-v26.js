...
export function unmount() {
  ...
  document.body.classList.remove("v26-exam-active", "v26-exam-nav-open");
}
...
function draw(container) {
  ...
  container.innerHTML = `<main class="v26-exam-shell"><header class="v26-exam-top"><div><span>SnowPro Core · COF-C03</span><strong>${modeName(session.mode)}</strong></div><div class="v26-exam-status"><div class="v26-save-status"><span>Status</span><strong data-save-state>${state.saving ? "Saving…" : "Saved"}</strong></div><div class="v26-time-status"><span>Time remaining</span><strong data-timer>${clock(remaining())}</strong></div></div><button type="button" data-submit>Submit Exam</button></header><div class="v26-exam-body"><aside class="v26-exam-nav" aria-label="Exam navigator"><div class="v26-exam-nav-head"><div><span>Exam Navigator</span><strong>${questions.length - answered} remaining</strong></div><button type="button" data-close-nav aria-label="Close exam navigator">×</button></div><div class="v26-nav-filters">${filter("all", "Overview", questions.length)}${filter("flagged", "Flagged", flagged)}${(state.config.domains || []).map((d, i) => domainFilter(d, i)).join("")}</div><div class="v26-question-grid">${filteredQuestions().map((item) => questionButton(item, questions.indexOf(item))).join("") || `<p>No questions in this view.</p>`}</div><div class="v26-exam-legend"><span><i class="done"></i>Answered</span><span><i class="flag"></i>Flagged</span></div><button class="v26-submit-side" type="button" data-submit>Submit Exam</button></aside><section class="v26-question-pane"><button class="v26-open-nav" type="button" data-open-nav>Exam Navigator</button><div class="v26-question-meta"><div><span>Question ${String(q.position).padStart(2, "0")} of ${questions.length}</span><strong><i style="--domain:${domainColor(domain.index)}"></i>${escape(domain.title)}${q.skill_id && q.skill_id !== "unmapped" ? ` · ${escape(q.skill_id.replaceAll("-", " "))}` : ""}</strong></div><button class="v26-flag-btn ${q.flagged ? "active" : ""}" type="button" data-flag aria-pressed="${q.flagged ? "true" : "false"}">${q.flagged ? "⚑ Flagged" : "⚐ Flag for review"}</button></div><article class="v26-question-card"><div class="v26-question-watermark" aria-hidden="true">${String(q.position).padStart(3, "0")}</div><p class="v26-question-kind">${q.multiple ? "Select all that apply." : "Select one answer."}</p>${context ? `<div class="v26-question-context"><span>Scenario</span><p>${escape(context)}</p></div>` : ""}<h1>${escape(stem)}</h1><fieldset><legend class="sr-only">Answer choices</legend>${(q.options || []).map((option, index) => answer(q, option, index)).join("")}</fieldset></article><footer class="v26-exam-footer"><button type="button" data-prev ${state.index === 0 ? "disabled" : ""}>← Previous</button><div class="v26-progress-dots" aria-label="Question progress">${questions.map((item, index) => `<button type="button" class="${index === state.index ? "current" : ""} ${item.selected?.length ? "done" : ""} ${item.flagged ? "flagged" : ""}" data-jump="${index}" aria-label="Question ${index + 1}${item.selected?.length ? ", answered" : ""}${item.flagged ? ", flagged" : ""}"></button>`).join("")}</div><button type="button" data-next ${state.index === questions.length - 1 ? "disabled" : ""}>Next →</button></footer></section></div><dialog class="v26-submit-dialog" data-dialog><div><p class="v26-kicker">Finish sitting</p><h2>Submit exam?</h2><div class="v26-submit-stats"><span><strong>${answered}</strong>Answered</span><span><strong>${questions.length - answered}</strong>Unanswered</span><span><strong>${flagged}</strong>Flagged</span></div><p>${questions.length - answered ? "Unanswered questions will be scored as incorrect." : "You answered every question."}</p><footer><button type="button" data-cancel>Continue Exam</button><button type="button" class="primary" data-confirm>Submit Exam</button></footer></div></dialog></main>`;
  bind(container);
}
...
function questionButton(q, index) { return `<button class="${index === state.index ? "current" : ""} ${q.selected?.length ? "done" : ""} ${q.flagged ? "flagged" : ""}" type="button" data-question-index="${index}" aria-label="Question ${q.position}${q.selected?.length ? ", answered" : ""}${q.flagged ? ", flagged" : ""}">${q.position}</button>`; }
...
function domainColor(index) { return DOMAIN_COLORS[index % DOMAIN_COLORS.length]; }
