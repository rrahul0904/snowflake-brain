...
function listHtml(items) {
  return `<ul class="guide-trap-list">${(items || []).map((item) => `<li>${escapeHtml(item)}</li>`).join("")}</ul>`;
}
...
async function renderSkill(container, trackId, skillId) {
  ...
  container.innerHTML = `<main class="guide-page">
    ...
    <section class="guide-content-block"><h2>What You Need to Know</h2>${listHtml(content.what_you_need_to_know || [skill.objective])}<div class="guide-key-concept"><strong>Key Concept</strong><p>${escapeHtml(content.key_concept || skill.objective || "")}</p></div></section>
    <section class="guide-content-block"><h2>Decision Rules</h2>${decisionRules(content.decision_rules || [])}</section>
    <section class="guide-content-block"><h2>Common Anti-Patterns</h2>${listHtml(content.anti_patterns || skill.exam_traps || [])}</section>
