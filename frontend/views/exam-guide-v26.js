...
export default async function render(root,params={}){
 ...
 const catalog=await api("/api/skills/catalog");
 const cert=(catalog.official_certifications||catalog.certifications||[]).find(x=>x.id===track)||(catalog.official_certifications||catalog.certifications||[])[0];
 if(!cert) throw new Error("Certification track is not configured.");
 const domains=cert.exam_domains||cert.domains||[];
 ...
 <section class="v26-section"><div class="v26-section-heading"><p class="v26-kicker">Blueprint map</p><h2>Start with the verified exam domains</h2></div>
 ${domains.map((d,di)=>`<article class="v26-diagnostic-card"><p class="v26-kicker">Domain ${di+1} · ${Number(d.weight||0)}%</p><h2>${escapeHtml(d.title||d.name||"Exam domain")}</h2><p>${escapeHtml(d.description||"Sign in to open the mapped task lessons, exam traps, concept checks and build exercises for this domain.")}</p><div class="v26-result-actions"><a class="v26-btn secondary" href="#/curriculum?track_id=${encodeURIComponent(track)}">Open mapped tasks</a><a class="v26-btn secondary" href="#/practice?track_id=${encodeURIComponent(track)}&mode=drill">Concept check</a><a class="v26-btn secondary" href="#/labs?track_id=${encodeURIComponent(track)}">Build coach</a></div></article>`).join("")}
 </section>
