...
function render(name, data) {
  if (name === "overview") return `<div class="admin-statusbar"><span>UTC reporting</span><span>Database: ${text(data.system.database)}</span><span>Billing: ${text(data.system.billing)}</span><span>Runtime: ${text(data.system.runtime_role)}</span></div>${cards([["Registered users", number(data.users.total), `${number(data.users.today)} today · ${number(data.users.seven_days)} in 7d`], ["Active learners", number(data.users.active_seven_days), `${number(data.users.active_today)} today`], ["Active subscriptions", number(data.subscriptions.active), `${money(data.subscriptions.mrr)} MRR`], ["Operating cost", money(data.finops.monthly_cost), data.finops.evidence.join(", ")]])}<div class="admin-grid"><article class="admin-panel"><h2>Learning activity</h2>${details(data.learning)}</article><article class="admin-panel"><h2>Release readiness</h2>${details(data.question_bank)}</article></div>`;
  if (name === "registrations") return `<h2>Registrations <small>${text(data.period)} · ${text(data.reporting_timezone)}</small></h2>${table(["date", "registrations"], data.rows)}`;
  if (name === "users") return `<h2>Users <small>${number(data.total)} total</small></h2>${table(["id", "created_at", "status", "auth_provider", "plan", "questions_used", "mock_count", "last_login_at"], data.rows)}`;
  if (name === "subscriptions") return `${cards([["Active subscribers", number(data.kpis.active_subscribers)], ["MRR", money(data.kpis.mrr)], ["ARR", money(data.kpis.arr)]])}${table(["candidate_id", "provider_customer_id", "provider_subscription_id", "internal_plan", "status", "current_period_end", "mrr_contribution"], data.rows)}`;
  ...
  if (name === "auth") return `<h2>Authentication operations</h2>${cards([["Registrations today", number(data.registrations_today)], ["Google users", number(data.google_users)], ["Password users", number(data.password_users)]])}<article class="admin-panel">${details({ sensitive_logs_excluded: data.sensitive_logs_excluded })}</article>`;
  if (name === "system") return `<article class="admin-panel">${details(data)}</article>`;
  if (name === "database") return `<h2>Database health</h2><article class="admin-panel">${details(data)}</article>`;
  if (name === "deployments") return `<p class="admin-note">Cost data: ${text(data.cost_data)}. This console cannot create a deployment.</p>${table(["deployment_sha", "environment", "status", "source_branch", "created_at"], data.rows)}`;
  if (name === "audit") return `<h2>Audit log <small>${number(data.total)} events</small></h2>${table(["event", "target_type", "target_id", "result", "created_at"], data.rows)}`;
  if (name === "configuration") return `<h2>Non-secret configuration</h2><article class="admin-panel">${details(data)}</article>`;
  return `<p class="admin-empty">Unknown administration section.</p>`;
}
