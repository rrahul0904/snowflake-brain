...
function planChoices(planCode, account, freeMock, quick, full) {
  const usage = account.membership?.usage || {};
  if (planCode === "free") {
    const weekly = usage.weekly_mocks || {};
    const remaining = weekly.remaining || 0;
    return [{ mode: "weekly-mock", title: "Free Full-Content Mock", questions: freeMock.question_count || 30, minutes: freeMock.time_limit_minutes || 45, allowance: remaining ? `1 weekly start available · resets ${resetText(weekly.resets_at)}` : `Weekly start used · resets ${resetText(weekly.resets_at)}`, disabled: !remaining, copy: "A timed 30-question exam spanning all five COF-C03 domains. A fresh eligible set is generated after the weekly reset." }];
  }
  if (planCode === "exam_pack_35") {
    const fullRemaining = usage.monthly_full_exams?.remaining || 0;
    return [
      { mode: "lifetime-practice", title: "100-Question Practice Mock", questions: full.question_count || 100, minutes: full.time_limit_minutes || 120, allowance: "Lifetime access · fixed candidate set", copy: "Repeat the same candidate-specific practice mock whenever you need it." },
  ...
  return [
    { mode: "quick-mock", title: "Quick Mock", questions: quick.question_count || 30, minutes: quick.time_limit_minutes || 45, allowance: `${daily.remaining ?? 0} daily questions remaining · resets ${resetText(daily.resets_at)}`, disabled: (daily.remaining ?? 0) < (quick.question_count || 30), copy: "A focused timed readiness check. Fresh question selection is applied as your daily allowance resets." },
    { mode: "full-mock", title: "Full-Length Mock", questions: full.question_count || 100, minutes: full.time_limit_minutes || 120, allowance: monthly.limit == null ? "Unlimited starts · fresh rotation enforced" : `${monthly.remaining ?? 0} monthly starts remaining · resets ${resetText(monthly.resets_at)}`, disabled: monthly.remaining === 0, copy: "The complete certification simulation using your monthly Full Exam allowance." },
  ];
}
...
function resetText(value) { if (!value) return "on its plan schedule"; const date = new Date(value); return new Intl.DateTimeFormat(undefined, { month: "short", day: "numeric", hour: "numeric", minute: "2-digit", timeZone: "UTC", timeZoneName: "short" }).format(date); }
