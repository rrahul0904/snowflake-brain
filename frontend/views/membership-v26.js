...
const plans = [
  { code: "free", name: "Free", price: "$0", cadence: "forever", label: "Build your foundation", copy: "Study materials, daily practice, and a weekly full-content timed mock after candidate sign-in.", features: ["All SnowPro Core study materials", "20 real practice questions every day", "One 30-question full-content timed mock every week", "Weekly mock resets automatically and receives a fresh eligible question set", "Weekly mock must be completed and cannot be discarded", "Progress, bookmarks, notes, drills, and exercises"] },
  { code: "premium_20", name: "Premium 100", price: "$20", cadence: "per month", label: "Consistent exam preparation", copy: "A daily 100-question allowance plus two full timed exam starts each month.", features: ["Everything in Free", "100 practice and Quick Mock questions per day", "2 full timed exams per month", "Daily and monthly allowances reset automatically", "Fresh eligible mock rotation across reset windows", "Saved sittings, history, results, and analytics"] },
  { code: "premium_40", name: "Premium 250", price: "$40", cadence: "per month", label: "High-volume preparation", copy: "More daily repetition and four full timed exam starts each month.", features: ["Everything in Free", "250 practice and Quick Mock questions per day", "4 full timed exams per month", "Daily and monthly allowances reset automatically", "Fresh eligible mock rotation across reset windows", "Saved sittings, history, results, and analytics"] },
  { code: "premium_100", name: "Premium 500", price: "$100", cadence: "per month", label: "Maximum preparation", copy: "The largest daily practice allowance with unrestricted full timed exam starts.", features: ["Everything in Free", "500 practice and Quick Mock questions per day", "Unlimited full timed exams", "Daily practice resets automatically", "Fresh eligible mock rotation minimizes immediate repeats", "Saved sittings, history, results, and analytics"] },
  { code: "exam_pack_35", name: "One-Time Exam Pack", price: "$35", cadence: "one-time", label: "No subscription", copy: "Keep the 100-question Practice Mock and use one Full Exam within the purchase window.", features: ["Lifetime access to a 100-question Practice Mock", "Candidate-specific fixed Practice Mock question set", "1 full timed exam attempt", "Full Exam must be started within 30 days of purchase", "Exam Pack question sets are fixed rather than reset", "Lifetime Practice Mock remains after the 30-day window"] },
];
...
function usagePanel(account, current) {
  ...
  const items = [];
  if (daily.limit != null) items.push(["Questions today", `${daily.remaining}/${daily.limit} remaining · resets ${formatReset(daily.resets_at)}`]);
  if (planCode === "free") items.push(["30Q full-content mock", `${weekly.remaining}/${weekly.limit} remaining · resets ${formatReset(weekly.resets_at)}`]);
  if (planCode.startsWith("premium_")) items.push(["Full exams this month", monthly.limit == null ? "Unlimited · fresh rotation" : `${monthly.remaining}/${monthly.limit} remaining · resets ${formatReset(monthly.resets_at)}`]);
...
function formatReset(value) {
  if (!value) return "on plan schedule";
  return new Intl.DateTimeFormat(undefined, { month: "short", day: "numeric", hour: "numeric", minute: "2-digit", timeZone: "UTC", timeZoneName: "short" }).format(new Date(value));
}
